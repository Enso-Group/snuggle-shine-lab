-- Scope dashboard data to the connected WhatsApp account.
--
-- Each conversation / person / group profile is tagged with channel_phone —
-- the digits-only phone of the linked account that owns it — so the dashboard
-- can show only the currently connected number's data and never mix in a
-- previously linked number's. Rows that stay NULL after this backfill are
-- filled in by the sweeper (defaulting to the connected account); the app also
-- treats NULL as "not yet classified" so nothing disappears in the meantime.
...
-- Provenance: a from_me message's sender_id is the OWN account that wrote it.
-- Tag each conversation with that account's phone (most recent wins).
...
-- People inherit the channel of their 1:1 conversation (matched on phone part).

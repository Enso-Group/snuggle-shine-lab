...
יש לך גישה מלאה למסד הנתונים ולחיבור Whapi דרך כלים:
- search_conversations(query): חפש שיחות/קבוצות לפי שם או חלק משם (גם בקבוצות שעוד לא נשמרו במסד).
- list_conversations(limit): רשימת השיחות האחרונות.
- get_messages(chat, limit): הודעות אחרונות משיחה מסוימת. אפשר להעביר whapi_chat_id או שם קבוצה. ברירת מחדל 30, מקסימום 200.
- stats(): סטטיסטיקות כלליות.

כללי עבודה חשובים:
1. כשהמשתמש מזכיר שם קבוצה (גם בתוך מרכאות "..." או ״...״) — חלץ את השם המדויק ותקרא איתו ל-search_conversations. אל תכניס לתוך השאילתה משפט ארוך או הוראות.
2. אם search_conversations מחזיר מועמדים — בחר את הכי תואם וקרא ל-get_messages עם ה-whapi_chat_id שלו.
3. אם בקריאה הראשונה לא נמצאה התאמה — נסה שוב עם וריאציות (בלי מרכאות, בלי "ה" בהתחלה, רק חלק מהמילים).
4. כשמבקשים ניתוח/פרופיל של משתתפים — קודם תביא הודעות (get_messages עם limit גבוה, למשל 200), ואז תנתח לפי שם השולח ותחזיר פרופיל לכל אחד עם דוגמאות ותובנות.
5. החזר תשובות בפורמט קריא ומסודר. אל תמציא נתונים — אם משהו חסר, אמור זאת.`;
    } else {
      systemPrompt =

...
function InstructionsPage() {
  const webhookUrl =
    typeof window !== "undefined" ? `${window.location.origin}/api/public/whapi-webhook` : "";
  ...
  return (
    <div className="min-h-full">
      <PageHeader
        icon={BookOpen}
        title="Instructions"
        description="Setup and Whapi connection instructions"
      />
      ...
      <PageContent maxWidthClass="max-w-3xl">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              The token is stored securely on the server (it can't be viewed from the browser)
            </CardDescription>
          </CardHeader>
            ...
            <NumberedList
              items={[
                ...
                <>
                  ...
                  <code className="rounded bg-muted px-1.5 py-0.5 text-xs">WHAPI_TOKEN</code> using
                  the "Add secret" button below
                </>,
                <>In Whapi → Settings → Webhook, enter this URL:</>,
              ]}
            />
            <div
              className="rounded-lg border bg-muted/40 p-3 font-mono text-xs break-all"
              dir="ltr"
            >
              {webhookUrl}
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm leading-relaxed">
            <p>
              To see all the messages from the phone (and not only those saved in the database),
              enable full history and then reconnect WhatsApp.
            </p>
            <p>
              When full history is active, it starts pulling old messages only after WhatsApp is
              reconnected. The group updates automatically every 10 seconds.
            </p>
            <p className="text-muted-foreground">
              The actions (enabling full history / reconnecting) are performed on the{" "}
              <strong>Participants</strong> page.
            </p>
          </CardContent>
        </Card>
      </PageContent>
